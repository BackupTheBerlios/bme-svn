			...
			else if (command == NotificationMessages::K_NS_DOWN)
			{
				...
				if(message->FindString(K_REMAINING_MSG,0,&nsString) == B_OK)
				{
					...
					else if (nsString == "SB")
					{
						...
						if(message->FindString(K_REMAINING_MSG,1,&sbServerAddress) == B_OK)
						{
							//separate Notification Server address and port
							int32 colonIndex = sbServerAddress.FindFirst(":");
							int sbPort;							
							if (colonIndex != B_ERROR)
							{								
								BString sbPortString;
								sbServerAddress.CopyInto(sbPortString, colonIndex + 1,sbServerAddress.CountChars() - colonIndex);
								sbPort = atoi(sbPortString.String());
								sbServerAddress.Remove(colonIndex,sbServerAddress.CountChars() - colonIndex);	
								//check authentication type
								BString authType;
								if (message->FindString(K_REMAINING_MSG,2,&authType) == B_OK)
								{
									if (authType == "CKI")
									{
										BString authenticationString;
										if (message->FindString(K_REMAINING_MSG,3,&authenticationString) == B_OK)
										{
											//connect to Switchboard Server
											BMessage *sbMessage = new BMessage(InterfaceMessages::K_CONNECT_SB_MSG);
											sbMessage->AddString(K_SB_ADDRESS,sbServerAddress);
