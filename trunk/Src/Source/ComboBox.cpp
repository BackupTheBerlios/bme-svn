...
{	
	...
	/*else if (!m_comboTextView->IsFocus()) 
	{
		m_comboTextView->MakeFocus(true);
		m_comboTextView->SelectAll();
	}*/
}

void ComboBox::MakeFocus(bool inFocus)
{
...
void ComboPopUpWindow::MessageReceived(BMessage *message)
{
	switch(message->what)
	{
		...
		{			
			BStringItem* item = (BStringItem*)m_combopopupview->ItemAt(m_combopopupview->CurrentSelection());						
		...
		{
			Lock();
			m_combopopupview->Select(m_combopopupview->CurrentSelection() + 1);
